...
# Estilos CSS personalizados para replicar la identidad visual de tu app original
st.markdown("""
    <style>
...
with tab_ingreso:
    ...
    with st.form("form_alta_lote", clear_on_submit=True):
        ...
        if btn_alta:
            ...
            append_row("Stock", [next_id, in_campana, in_especie, in_variedad, in_categoria, in_deposito, in_bolsas, in_kilos_b, kilos_t, in_estado, in_notes if 'in_notes' in locals() else in_notas])
            append_row("Historial", [now_str, "INGRESO", f"Alta lote ID {next_id}: {in_bolsas} bolsas de {in_variedad} en {in_deposito}", in_bolsas, kilos_t, "Ignacio Diaz"])
...
with tab_movimiento:
    ...
    if df_stock.empty or (df_stock['Bolsas'] == 0).all():
        st.warning("No hay existencias físicas en la base de datos para realizar movimientos.")
    else:
        # Armar lista desplegable amigable con los lotes reales en la nube
        opciones_lotes = []
        ...
        if st.button("⚡ Ejecutar Operación y Actualizar Todas las PC", use_container_width=True):
            if modalidad == "Egreso por Despacho (Genera Orden de Carga)" and not cliente_oc:
                ...
                # Descontar del lote de origen directamente en la fila exacta de Google Sheets
                w_stock = sh.worksheet("Stock")
                celda_id = w_stock.find(str(lote_id_real))
                numero_fila = celda_id.row
                
                quedan_bolsas = int(datos_origen['Bolsas']) - cant_mover
                quedan_kilos = quedan_bolsas * float(datos_origen['Kilos_por_Bolsa'])
                
                w_stock.update_cell(numero_fila, 7, quedan_bolsas)  # Modifica Columna Bolsas (G)
                w_stock.update_cell(numero_fila, 9, quedan_kilos)   # Modifica Columna Kilos Totales (I)
                
                if modalidad == "Transferencia entre Plantas / Depósitos":
                    # Sumar en el depósito destino (Creando una nueva entrada para trazabilidad)
                    next_id_t = int(df_stock['ID'].max()) + 1
                    append_row("Stock", [
                    ...
                    append_row("Historial", [now_str, "TRANSFERENCIA", f"Traspaso de {cant_mover} bols de {datos_origen['Variedad']} desde {datos_origen['Depósito']} hacia {destino_dep}", cant_mover, kilogramos_mov, "Ignacio Diaz"])
                    st.success(f"¡Transferencia exitosa! Las existencias se reubicaron en {destino_dep}.")
                    st.rerun()
                else:
                    # Registrar Egreso y Generar Orden de Carga Correlativa
                    proxima_oc = int(df_ordenes['ID_Orden'].max()) + 1 if not df_ordenes.empty else 5001
                    append_row("Ordenes", [
                    ...
                    append_row("Historial", [now_str, "EGRESO", f"Despacho OC #{proxima_oc}: {cant_mover} bols entregadas a {cliente_oc}", cant_mover, kilogramos_mov, "Ignacio Diaz"])
                    
                    # Generación automatizada de mensaje de WhatsApp estructurado profesionalmente
                    texto_wa = (
...
with tab_historial:
    ...
    if df_historial.empty:
        ...
        # Invierte el orden para ver lo último que se hizo arriba de todo
...
# 8. PIE DE PÁGINA REQUERIDO (Propiedad e interfaz blindada)
st.markdown("""
    <div class="footer">
